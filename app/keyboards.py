...
#schedule = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='Розклад')]], resize_keyboard=True)
schedule = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text='Розклад', callback_data='schedule')],
    #[InlineKeyboardButton(text='Змінити групу', callback_data='')]
])
...
async def groups(specialty: str):
    ...
    for group in all_groups:
        keyboard.add(InlineKeyboardButton(text=group, callback_data=f"group_{specialty}-{group}"))
    return keyboard.adjust(1).as_markup()
